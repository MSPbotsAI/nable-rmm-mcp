...
from nable_rmm_mcp.__main__ import _build_http_app
from nable_rmm_mcp.config import Settings
from nable_rmm_mcp.server import create_mcp_server, get_client_from_context
...
def test_missing_header_returns_401_with_required_headers_listed():
    ...
    with TestClient(app) as client:
        ...
        body = resp.json()
        assert "X-Nable-Api-Token" in body["required_headers"]
...
def test_header_present_reaches_request_context(monkeypatch):
    ...
    from nable_rmm_mcp.server import GatewayTokenMiddleware, _gateway_creds_var
    ...
    async def run():
        scope = {
            ...
            "path": "/mcp",
            "headers": [
                (b"x-nable-api-token", b"test-key-123"),
                (b"x-nable-server", b"https://www.systemmonitor.co.uk"),
            ],
        }
    ...
    asyncio.run(run())
    assert seen["creds"] == ("test-key-123", "https://www.systemmonitor.co.uk")
